...
def extract_candidate(segment: dict, index: int) -> dict:
    ...
    required_action = first_present(text, [
        "동의를 받아야 한다",
        "확인하여야 한다",
        "수행한다",
        "승인을 받아야 한다",
    ...
    prohibition = first_present(text, [
        "포함해서는 아니 된다",
        "처리해서는 아니 된다",
    ])
    exception = first_present(text, [
        "동의 없이",
        "다만",
        "제28조의2에도 불구하고",
    ])
    data_type = first_present(text, [
        "가명정보",
        "개인신용정보",
        "특정 개인을 알아볼 수 있는 정보",
    ])
    applies_to = first_present(text, [
        "개인정보처리자",
        "신용정보제공ㆍ이용자",
        "신용조회회사",
        "신용정보집중기관",
    ...
    review_status = "PENDING" if any([required_action, prohibition, exception]) else "REVIEW_REQUIRED"
    return {
...
def main() -> None:
    ...
    candidates = [extract_candidate(segment, index) for index, segment in enumerate(parsed, start=1)]
