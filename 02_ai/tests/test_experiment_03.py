...
def test_method_matrix_is_frozen_before_results_and_source_bound() -> None:
    ...
    assert source_types >= {"OFFICIAL_GUIDANCE", "FINANCIAL_OPERATION_REFERENCE", "SECURITY_STANDARD", "PEER_REVIEWED"}
    assert {row["method"] for row in matrix["rows"]} == {
        "KEEP", "REMOVE", "MASK", "HMAC_PSEUDO", "VAULT_TOKEN", "STANDARD_ENCRYPTION",
        "FPE", "OPE_ORE", "GENERALIZE", "AGGREGATE", "NOISE_RANDOMIZATION",
    }
...
def test_e3_is_workload_bound_and_fail_closed(built_artifacts) -> None:
    ...
    assert validation["field_count"] == 20
    assert all(set(row["operational_metrics"]) == {
        "transform_latency_ms", "payload_size_before", "payload_size_after", "schema_validity",
        "deterministic_consistency", "transform_failure", "retrieval_compatibility", "outbound_compatibility",
    } for row in results["results"])


def test_current_workload_mask_variants_fail_combined_privacy_and_relation_fit(built_artifacts) -> None:
    handoff, matrix, results = built_artifacts
...
def test_e2_revision_keeps_only_exact_preserving_amount_candidate() -> None:
    handoff = load_handoff(ARTIFACT_DIR / "E2_TO_E3_TRANSFORM_REQUIREMENTS.json")
    rows = [row for row in handoff["requirements"]
    ...
    assert all(row["approved_requirement"] == "KEEP_ONLY" for row in handoff["revision"]["field_revisions"])
...
def test_overall_fit_cannot_override_failed_required_gate(built_artifacts) -> None:
    ...
    row["overall_fit"] = "FIT"
    row["evidence_digest"] = canonical_digest({key: value for key, value in row.items() if key != "evidence_digest"})
    changed["bundle_digest"] = canonical_digest({key: value for key, value in changed.items() if key != "bundle_digest"})
...
def test_validated_profile_activates_exact_value_runtime_contract(built_artifacts) -> None:
    ...
    exact = [row for row in profile["profiles"]
             if row["field_name"] in {"account.balance", "transaction.amount"}]
...
def test_integrated_profile_keeps_external_execution_pending(built_artifacts) -> None:
    ...
    integrated = build_integrated_validation_profile(handoff, profile, rerun, "sha256:rag", "sha256:controls")
    assert validate_integrated_validation_profile(integrated)["status"] == "PASS"
    assert integrated["components"]["e2_policy_requirement_validation"]["external_model_execution"] == "PENDING_EXTERNAL_EXECUTION"


def test_da_be_fe_governance_contract_uses_same_digests_and_fields(built_artifacts) -> None:
    artifact_dir = Path(__file__).parents[1] / "artifacts/experiment_03"
    handoff = json.loads((artifact_dir / "E2_TO_E3_TRANSFORM_REQUIREMENTS.json").read_text(encoding="utf-8"))
    profile = json.loads((artifact_dir / "E3_VALIDATED_TRANSFORM_PROFILE.json").read_text(encoding="utf-8"))
    workspace = Path(__file__).parents[3]
    be_projection = (workspace / "ADP-BE/src/main/java/com/adp/gateway/ai/application/"
                      "AiTransformGovernanceProfileService.java").read_text(encoding="utf-8")
    fe_projection = (workspace / "ADP-FE/src/features/ai-evaluation/mocks/handlers.ts").read_text(encoding="utf-8")
...
def test_provider_governance_baseline_is_active_fail_closed_and_cross_repo_bound() -> None:
    governance = json.loads((ARTIFACT_DIR / "E2_PROVIDER_GOVERNANCE_BASELINE.json").read_text(encoding="utf-8"))
    workspace = Path(__file__).parents[3]
    be_contract = (workspace / "ADP-BE/src/main/java/com/adp/gateway/ai/application/"
                   "AiEvaluationContractService.java").read_text(encoding="utf-8")
    fe_projection = (workspace / "ADP-FE/src/features/ai-evaluation/mocks/handlers.ts").read_text(encoding="utf-8")
    ...
    mapping = {row["model_profile_id"]: row["destination_digest"]
               for row in governance["destination_contract_mapping"]}
    assert mapping == {
        "google-gemma-4-31b-it": "sha256:824292c9ad5bdcd8cd1c502997053243f179782e18402ca380b4c54a0c76f991",
        "meta-muse-glimmer-30b": "sha256:35827b9982c8813acef21a321d0bde08d856039a882c09cbf764d72d1cc3b607",
        "nvidia-nemotron-3.5-lightning-30b-a3b": "sha256:a6d7fb15ca4da646ca16fb92eb2a7c39327be9cdff21a130012f564749bc922e",
    }
...
def test_holdout_baseline_digest_and_readiness_are_fail_closed() -> None:
    baseline = json.loads((ARTIFACT_DIR / "AI_E1_E2_E3_HOLDOUT_BASELINE.json").read_text(encoding="utf-8"))
    integrated = json.loads((ARTIFACT_DIR / "AI_INTEGRATED_VALIDATION_PROFILE.json").read_text(encoding="utf-8"))
