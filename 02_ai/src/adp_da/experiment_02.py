...
RUNTIME_STAGES = {
    "Authentication", "Authorization", "WorkloadPurposeSubjectActionBinding", "Policy",
    "Retrieval", "Transform", "OutboundGuard", "Provider", "ResponseGuard",
    "ControlledDelivery", "ContractEvaluationBundle", "StageTimingTrace",
}
...
E2_RELEASED_FIELDS = {
    "input.prompt", "customer.customer_id", "customer.segment", "account.account_id",
...
REQUIRED_REQUIREMENT_FIELDS = {
    "source_id", "source_title", "source_type", "authority", "effective_date",
    "article_or_section", "requirement_id", "requirement_text", "applicability_condition",
    "applicability", "workload_scope", "data_scope", "provider_scope", "destination_scope",
    "fpg_control_id", "runtime_stage", "expected_decision", "evidence_field", "citation",
    "content_digest",
}
REQUIRED_HANDOFF_ROW_FIELDS = {
    "case_id", "workload_id", "purpose_code", "field_name", "business_need",
    "regulatory_requirement_ids", "internal_policy_ids", "applicability",
    "field_requirement", "transform_intent", "utility_requirement",
    "candidate_transform_methods", "prohibited_transform_methods", "required_exact",
    "relation_preservation_required", "reversibility_allowed", "external_release_allowed",
    "runtime_control_id", "decision_reason", "evidence_digest",
}
...
def validate_e2_to_e3_handoff(value: dict[str, Any]) -> dict[str, Any]:
    ...
    _fail(value.get("provider_call_authorized") is not False, "handoff provider authorization must be false")
    ...
    _fail(any(not REQUIRED_HANDOFF_ROW_FIELDS <= row.keys() for row in rows), "handoff row field missing")
    _fail(any(row.get("workload_id") != "customer_summary" or row.get("purpose_code") != "CUSTOMER_SUPPORT"
              for row in rows), "handoff workload/purpose mismatch")
    ...
    _fail(any(row.get("field_requirement") != "REQUIRED_EXACT" or row.get("required_exact") is not True
              or row.get("candidate_transform_methods") != ["KEEP"]
              or "GENERALIZE" not in row.get("prohibited_transform_methods", [])
              for row in exact_amount_rows), "handoff KEEP_ONLY requirement mismatch")
    return {
        "status": "PASS",
...
def validate_evidence(value: dict[str, Any]) -> dict[str, Any]:
    ...
    _fail(not sources or not requirements or len(cases) != 3, "source/requirement/positive-case cardinality mismatch")

    source_ids = {item.get("source_id") for item in sources}
    _fail(None in source_ids or len(source_ids) != len(sources), "source identity mismatch")
    _fail(any(item.get("source_type") not in SOURCE_TYPES for item in sources), "source type mismatch")
    ...
    for query in queries:
        ...
        _fail(any(item not in requirement_ids for item in query.get("ranked", [])), "RAG ranked binding mismatch")
    hits = {k: sum(q["gold"] in q["ranked"][:k] for q in queries) / len(queries) for k in (1, 3, 5)}
    expected = value["rag"]["metrics"]
    _fail(any(abs(hits[k] - expected[f"hit_at_{k}"]) > 1e-12 for k in (1, 3, 5)), "RAG metric mismatch")

    layers = value.get("verification_layers", {})
    _fail(set(layers) != {"source_coverage", "retrieval_correctness", "runtime_enforcement", "execution_evidence"},
          "four-layer verification mismatch")
    _fail(layers["source_coverage"] != "PASS" or layers["retrieval_correctness"] != "PASS",
          "corpus verification layer mismatch")
    _fail(layers["runtime_enforcement"] not in {"PASS", "PENDING"}
          or layers["execution_evidence"] not in {"PASS", "PENDING"}, "runtime verification layer mismatch")
    _fail(layers["runtime_enforcement"] == "PASS" and layers["execution_evidence"] != "PASS",
          "runtime enforcement requires execution evidence")

    negatives = value.get("negative_cases", [])
    _fail(len(negatives) != 4 or any(item.get("provider_calls") != 0 for item in negatives),
          "negative provider-call-zero mismatch")
    _fail(any(key in json.dumps(value).lower() for key in ('"raw_value"', '"raw_text"', '"offset"')),
    ...
    return {
        "status": "PASS", "source_count": len(sources), "requirement_count": len(requirements),
        "runtime_mapped": len(requirements), "unmapped": 0,
...
def build_rag_top1_analysis(value: dict[str, Any]) -> dict[str, Any]:
    ...
    misses = []
    definitions = (
        ("Q04", "EFSR-CLOUD-IMPORTANCE", "PIPA-OVERSEAS-TRANSFER",
         "INCORRECT_TOP1_DIFFERENT_REQUIREMENT",
         "Cloud service importance assessment is adjacent destination governance, but it does not answer the overseas-transfer legal-condition query."),
        ("Q06", "AI-ACT-HIGH-IMPACT-CHECK", "CREDIT-AUTOMATED-EVALUATION",
         "INCORRECT_TOP1_DIFFERENT_REQUIREMENT",
         "High-impact AI classification is adjacent applicability context, but it does not answer the Credit Information Act automated-evaluation rights query."),
    )
    for query_id, previous_top1, gold, assessment, reason in definitions:
        ...
        misses.append({
            ...
            "analysis": reason,
            "runtime_decision_impact": "NONE; both queries remain evidence retrieval inputs and applicability is evaluated independently.",
...
def validate_rag_top1_analysis(value: dict[str, Any], evidence: dict[str, Any]) -> dict[str, Any]:
    _fail(value.get("schema_version") != RAG_TOP1_ANALYSIS_SCHEMA, "RAG Top1 analysis schema mismatch")
    ...
    _fail(value.get("status") != "RESOLVED" or len(value.get("misses", [])) != 2,
          "RAG Top1 misses are not resolved")
    ranked = {row["query_id"]: row for row in evidence["rag"]["queries"]}
    _fail(any(ranked[row["query_id"]]["ranked"][0] != row["expected_gold_requirement"]
              or row["resolved_gold_rank"] != 1 for row in value["misses"]),
          "RAG Top1 resolution does not match frozen rankings")
    _fail(value.get("after_metrics") != {"hit_at_1": 1.0, "hit_at_3": 1.0, "hit_at_5": 1.0},
...
def build_internal_control_roles(controls: list[dict[str, Any]]) -> dict[str, Any]:
    direct = {"CTRL-EVAL-001", "CTRL-RUNTIME-001", "CTRL-RUNTIME-003", "CTRL-RUNTIME-008",
              "CTRL-GOV-003", "CTRL-RUNTIME-009"}
    policy_input = {"CTRL-RUNTIME-002", "CTRL-RUNTIME-004", "CTRL-RUNTIME-005",
                    "CTRL-RUNTIME-006", "CTRL-RUNTIME-007"}
    explicit_roles = {
        "CTRL-GOV-001": "AUDIT_ONLY", "CTRL-GOV-002": "HUMAN_REVIEW",
        "CTRL-REF-001": "HUMAN_REVIEW", "CTRL-REF-002": "AUDIT_ONLY",
        "CTRL-GOV-004": "GOVERNANCE_ONLY", "CTRL-REF-003": "HUMAN_REVIEW",
        "CTRL-GOV-005": "GOVERNANCE_ONLY", "CTRL-GOV-006": "PROVIDER_GOVERNANCE",
    }
    runtime_locations = {
    ...
    for control in controls:
        ...
        if control_id in direct:
            ...
            location = "Policy/applicability input; no independent final Runtime allow/block binding in E2"
        else:
...
def validate_internal_control_roles(value: dict[str, Any]) -> dict[str, Any]:
    _fail(value.get("schema_version") != INTERNAL_CONTROL_ROLE_SCHEMA, "internal control role schema mismatch")
    ...
    _fail(len(rows) != 19 or len({row.get("internal_control") for row in rows}) != 19,
    ...
    _fail(dict(counts) != {"DIRECT": 6, "INDIRECT": 5, "GOVERNANCE": 8},
          "internal control role-family counts mismatch")
    _fail(any(not row.get("runtime_api_trace_location") or not row.get("evidence") for row in rows),
          "internal control role explanation missing")
    return {"status": "PASS", "control_count": 19,
            "role_family_counts": value["role_family_counts"], "classification_digest": expected}
...
def validate_runtime(bundle: dict[str, Any], traces: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    _fail(any(len(values) != 1 for values in by_case.values()), "semantic provider input differs by model")
    ...
    for trace in traces:
        ...
        _fail(authorization == "DENIED" and provider != "NOT_CALLED", "provider called after authorization deny")
        _fail(policy == "BLOCK" and provider != "NOT_CALLED", "provider called after policy block")
        _fail(outbound == "BLOCKED" and provider != "NOT_CALLED", "provider called after outbound block")
        _fail(response_guard == "REJECTED" and delivery == "DELIVERED", "rejected response delivered")
        _fail(delivery == "WITHHELD" and bool(evidence.get("deliveredResponseDigest")), "withheld response exposed")
        _fail(not evidence.get("regulatoryEvidenceDigest") or not evidence.get("ragEvidenceDigest"),
              "regulatory/RAG evidence binding missing")
        timings = trace.get("stageTimings", [])
        _fail(not timings, "stage timing evidence missing")
        for timing in timings:
            _fail(timing["endedAt"] < timing["startedAt"] or timing["durationMillis"] < 0,
        ...
        _fail(all(isinstance(item, int) for item in tokens) and tokens[0] + tokens[1] != tokens[2],
              "token sum mismatch")
...
def validate_synthetic_egress_evidence(value: dict[str, Any]) -> dict[str, Any]:
    ...
    _fail("DURATION_UNSPECIFIED" not in terms.get("retention", ""), "provider retention uncertainty missing")
    _fail("AI_MODEL_IMPROVEMENT" not in terms.get("training_or_reuse", ""), "provider reuse disclosure missing")

    resolution = value.get("resolution", {})
    _fail(resolution.get("path") != "FULLY_SYNTHETIC_NON_LINKABLE_MINIMIZED_PAYLOAD",
          "synthetic resolution path mismatch")
    _fail(resolution.get("provider_conditions_treated_as_resolved") is not False,
          "provider uncertainty must remain explicit")
    _fail("MUST_NOT_CALL_PROVIDER" not in resolution.get("fail_closed_condition", ""),
          "synthetic evidence is not fail-closed")

    provenance = value.get("dataset_provenance", {})
    _fail(provenance.get("dataset_digest") != E2_DATASET_DIGEST, "synthetic dataset digest mismatch")
    _fail(provenance.get("source_type") != "SYNTHETIC" or provenance.get("real_person_linkage") != "NONE",
    ...
    _fail(set(payload.get("retrieved_and_released_field_paths", [])) != E2_RELEASED_FIELDS,
          "released field allowlist mismatch")
    _fail(set(payload.get("transform_treatments", {})) != E2_RELEASED_FIELDS,
          "released field transform coverage mismatch")
    _fail(any(payload.get(flag) is not False for flag in (
        "raw_personal_data_present", "raw_credit_information_present",
        "confidential_fpg_data_present", "linkable_to_real_person",
    )), "unsafe outbound data classification")

    reevaluation = value.get("reevaluation", {})
    _fail(reevaluation.get("applicability", {}).get("on_mismatch") != "UNRESOLVED_MUST_NOT_CALL_PROVIDER",
          "applicability mismatch is not fail-closed")
    _fail(reevaluation.get("policy", {}).get("decision") != "TRANSFORM"
    ...
    _fail(guard.get("decision") != "PASS_ELIGIBLE_NOT_EXECUTED"
    ...
    _fail(any(baseline.get(key) != 0 for key in (
        "provider_request", "connector_execution", "ai_model_execution_evidence",
...
def validate_temporal_provenance(value: dict[str, Any]) -> dict[str, Any]:
    _fail(value.get("schema_version") != "adp-ai-synthetic-temporal-provenance/v1",
          "temporal provenance schema mismatch")
    _fail(value.get("temporal_consistency_status") != "VERIFIED",
    ...
    _fail(len(rows) != 6 or len({row.get("transaction_id") for row in rows}) != 6,
    ...
    for row in rows:
        ...
        _fail(not lower <= normalized.date() <= reference, "normalized timestamp outside retrieval window")
        _fail(not row.get("customer_id") or not row.get("account_id")
              or not row.get("transaction_type") or not row.get("amount"),
              "preserved transaction semantics missing")
    required = {
        "customer_account_relationship", "transaction_amount", "transaction_type",
        "balance_relationship", "regulatory_trigger", "workload_purpose",
        "transform_obligation", "destination_contract",
    }
...
def build_operational_metrics_contract() -> dict[str, Any]:
    ...
    for case_id in E2_CASES:
        for model in E2_MODELS:
            ...
            executions.append({
                "case_id": case_id,
                "model": model,
                "pre_provider_execution_id": pre_provider_ids[case_id] if is_pre_provider else None,
    ...
    payload = {
        ...
        "fixed_conditions": [
            "CASE", "WORKLOAD", "PURPOSE", "REGULATORY_RAG", "INTERNAL_POLICY",
            "PROMPT", "RUNTIME_CONTRACT", "DESTINATION", "GENERATION_CONDITION",
        ],
        "measurement_sources": {
        ...
        "token_interpretation": "TOKEN_COUNTS_ARE_MODEL_TOKENIZER_SPECIFIC_AND_NOT_A_GLOBAL_EFFICIENCY_SCORE",
        "response_finding_contract": {
            "native_source": "BE responseFindingTypes and calibration-evidence finding_type",
            "native_types": [
                "RAW_VALUE_REFLECTION", "EMAIL", "PHONE_NUMBER", "ACCOUNT_NUMBER",
                "RESIDENT_REGISTRATION_NUMBER", "PRIVATE_KEY", "ACCESS_TOKEN",
                "REFRESH_TOKEN", "CREDENTIAL", "SECRET", "SEED",
            ],
            "analysis_mapping": {
                "SENSITIVE_DATA_EXPOSURE": "native sensitive finding types including RAW_VALUE_REFLECTION",
                "RESPONSE_GUARD_REJECTED": "response_guard_status/reason code; not a duplicate finding enum",
...
def validate_operational_metrics_contract(value: dict[str, Any]) -> dict[str, Any]:
    _fail(value.get("schema_version") != OPERATIONAL_METRICS_SCHEMA, "operational metrics schema mismatch")
    ...
    _fail(len(rows) != 9 or pairs != {(case, model) for case in E2_CASES for model in E2_MODELS},
          "operational metrics Cartesian contract mismatch")
    metric_fields = {
        "provider_latency_ms", "response_guard_latency_ms", "runtime_end_to_end_latency_ms",
        "total_execution_latency_ms", "input_tokens", "output_tokens", "total_tokens",
        "response_finding_types",
    }
    _fail(any(row.get(field) is not None for row in rows for field in metric_fields),
          "unexecuted metric must remain null")
    _fail(any(row.get("provider_execution_status") != "NOT_EXECUTED" for row in rows),
    ...
    _fail(mappings.get("UNSUPPORTED_CLAIM") != "NOT_IMPLEMENTED_AS_NATIVE_BE_FINDING",
...
def main() -> None:
    ...
    if args.write_operational_contract:
        ...
        print(json.dumps({"status": "PASS", "output": str(args.write_operational_contract),
                          "contract_digest": value["contract_digest"]}, indent=2))
        return
    ...
    print(json.dumps(validate_evidence(json.loads(args.evidence.read_text(encoding="utf-8"))), indent=2))
