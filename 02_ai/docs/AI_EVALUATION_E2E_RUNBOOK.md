...
2. 관리자 Readiness API에서 `READY`와 `bundle_available=true` 검증
3. Case×Model 진단을 포함한 `readiness.json` 보존
4. 관리자 Bundle API에서 원본 JSON export
5. DA의 Schema, digest, Cartesian Product, provenance 검증
6. 검증을 통과한 Bundle만 DA 분석 산출물로 변환
...
BE가 이미 세 모델을 실행했다면 Provider를 다시 호출하지 않고 Readiness부터 이어간다.

```bash
python -m adp_da.evaluation_e2e \
  --consume-existing \
  --session-id be-v25-existing-001 \
  --output outputs/real_be_evaluation/be-v25-existing-001
```

DA 컨테이너 안에서는 Compose가 `ADP_BE_BASE_URL=http://adp-be:8080`과 로컬 Runtime/Admin
인증 변수를 전달한다. 호스트에서 실행할 때는 `http://127.0.0.1:8080`을 사용한다.

`--session-id`는 재실행마다 새 값으로 지정한다. 같은 값은 BE idempotency replay가 목적일 때만
재사용한다. 출력 디렉터리가 이미 존재하면 덮어쓰지 않고 실패한다.

현재 BE에는 Bearer/JWT 인증 Adapter가 없으므로 원격 HTTPS 실행도 기본 차단된다. 운영 Adapter가
배포·검증된 뒤에만 `ADP_BE_TOKEN`과 `ADP_BE_REMOTE_BEARER_AUTH_ENABLED=YES`를 함께 설정한다.
이 확인값은 인증 기능을 구현하지 않으며 이미 배포된 Adapter 사용을 명시적으로 허용할 뿐이다.
로컬 Header를 운영 인증 대체로 사용하지 않는다. 원격 plain HTTP와 URL 내 credential은 거부된다.

## 3. 성공 산출물과 완료 조건

- `bundle_export/sha256-<raw digest>.json`: BE 응답 원본 바이트
- `readiness.json`: Bundle 요청 직전 Case×Model 준비 상태와 실패 진단
...
- `bundle_auth_present`: 지원되는 로컬 privileged Header가 없거나 검증된 원격 Adapter 설정이 없음
- `remote_bearer_adapter_confirmed`: 원격 Bearer Adapter 배포 확인값이 없음
...
- Readiness 실패: 보존된 `readiness.json`의 Case×Model별 상태 확인
- Bundle 404: Readiness 이후 provenance 변경과 institution/workload scope 확인
