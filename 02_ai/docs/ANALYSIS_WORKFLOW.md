...
분석 결과는 `02_ai/contracts/evaluation_artifact.schema.json` 형식으로 정리하고, 개발팀이 구현할 Requirement, Control, Test 후보를 함께 전달합니다.
...
AI Evidence and Requirement Candidate artifacts now live under
`02_ai/evidence_ontology`; AI Control, Test, and Policy Candidate artifacts now
live under `02_ai/gateway_rules`. Digital Asset artifacts are physically split
under `03_digital_asset/evidence_ontology` and
`03_digital_asset/gateway_rules`.

`02_ai/contracts/evaluation_artifact.schema.json` is validation evidence for actual
analysis and experiment results. `02_ai/contracts/policy_evaluation_artifact.schema.json`
